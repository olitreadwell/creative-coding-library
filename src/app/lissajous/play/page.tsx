...
export default function LissajousPage() {
  ...
  return (
    <main className="min-h-screen bg-[#06060e] text-foreground flex flex-col">
      <header className="px-4 sm:px-6 py-3 sm:py-4 flex flex-wrap items-center gap-3 border-b border-white/10">
        ...
        <h1 className="text-sm font-medium tracking-wide text-white/90 flex-1 min-w-0 truncate">
      ...
      <section className="flex-1 relative" aria-label="Animated Lissajous harmonograph">
        <canvas
          ...
          aria-label="Lissajous curve animation. Two sine waves combine to trace a glowing, morphing path."
          style={{ background: "#06060e" }}
        />
      </section>

      <footer className="px-4 sm:px-6 py-3 sm:py-4 text-xs text-white/70 border-t border-white/10">
        Technique: parametric sine curves with additive glow. Named after{" "}
        <a
          href="https://en.wikipedia.org/wiki/Lissajous_curve"
          target="_blank"
          rel="noopener noreferrer"
          className="underline underline-offset-2 hover:text-white/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 rounded"
        >
