...
export default function LSystemTreePage(): React.ReactElement {
  ...
  // Draw whenever seed, iterations, or canvas size changes
  useEffect(() => {
    const canvas = canvasRef.current;
    ...
    ctx.save();
    ctx.scale(dpr, dpr);
    drawTree(ctx, size, seed, iterations);
    ctx.restore();
  }, [seed, iterations, size]);
  ...
  return (
    <main className="min-h-screen bg-[#0d1117] text-white flex flex-col">
      <header className="px-6 py-4 flex items-center gap-4 border-b border-white/10 flex-wrap">
        ...
        <h1 className="text-sm font-medium tracking-wide text-white/80 mr-auto">L-System Tree</h1>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <label className="flex items-center gap-2 text-sm text-white/60">
            ...
            />
            <span className="w-4 text-right text-white/80">{iterations}</span>
          </label>
      ...
      <section className="flex-1 relative" aria-label="L-system fractal plant on a dark canvas">
        <canvas
          ref={canvasRef}
